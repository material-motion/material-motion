...
Here, `fadeIn` is the Expectation. The fading in of the target **is not executed here**. `addExpectation` has simply registered the Expectation of the desired change to some system. That system is expected to fulfill the Expectation at some later point in the program's execution.
