...
For example: if an executor does not care about the animation event then the Runtime should not inform the executor. If an executor wants to listen to a gesture recognizer, it should only receive events from that specific recognizer.
