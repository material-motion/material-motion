...
For example: if an executor does not care about the animation event then the Runtime should not inform the executor. If an executor wants to listen to events for a specific gesture recognizer, it should only have to listen to events from that particular recognizer.
