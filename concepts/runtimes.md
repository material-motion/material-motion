...
For example: if an executor does not care about the animation event then the Runtime should not inform the executor. If an executor cares about a specific gesture recognizer then send events from that recognizer.
